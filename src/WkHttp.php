...
class WkHttp
{
    ...
    {
        Worker::safeEcho("session Start\n");

        if (NO_CLI) {
            return \session_start();
        }
        Worker::safeEcho(HttpCache::$sessionType . "，session type\n");

        if (HttpCache::$sessionType == 'file') {
            self::tryGcSessions();
        }
print_r(HttpCache::$instance);
        if (HttpCache::$instance->sessionStarted) {
            Worker::safeEcho("already sessionStarted\n");
            return true;
        }
        HttpCache::$instance->sessionStarted = true;
        print_r(HttpCache::$sessionName);
        print_r($_COOKIE);
        // Generate a SID.
        if (!isset($_COOKIE[HttpCache::$sessionName])) {
            Worker::safeEcho("create sessionId\n");
        ...
        Worker::safeEcho(HttpCache::$instance->sessionFile . "\n");
    ...
    protected static function sessionRead()
    {
        if (HttpCache::$sessionType == 'file') {
            ...
            $handler = static::openRedis();
            $sessID = 'ses_' . md5(HttpCache::$instance->sessionFile);
    ...
    protected static function openRedis()
    {
...
defined('NO_CLI') or define('NO_CLI', \PHP_SAPI !== 'cli');
